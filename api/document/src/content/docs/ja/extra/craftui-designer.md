...
すべてのメソッドにはデフォルト実装が存在するため、カスタマイズしたいメソッドのみをオーバーライドすれば十分です。
...
設定をリセットしたい場合は `CustomCrafterAPI.setCraftUIDesigner(CustomCrafterAPI.DEFAULT_CRAFT_UI_DESIGNER)` を呼び出します。
...
class MyPlugin : JavaPlugin() {
    override fun onEnable() {
        // MAJOR_VERSION / MINOR_VERSION で必要バージョン (5.2.0 以上) を直接比較する
        if (CustomCrafterAPI.MAJOR_VERSION < 5 ||
            (CustomCrafterAPI.MAJOR_VERSION == 5 && CustomCrafterAPI.MINOR_VERSION < 2)) {
            logger.warning("CustomCrafterAPI 5.2.0 以上が必要です。(現在: ${CustomCrafterAPI.API_VERSION})")
...
:::note
`setCraftUIDesigner` を呼び出すと、内部的に `designer.bake(Context(player = null))` が実行され `isValid()` のチェックが行われます。
