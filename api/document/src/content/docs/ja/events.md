...
| `isAsync` | `Boolean` | 非同期スレッドからの呼び出しかどうか (5.2.0 以降) |

`registererMeta()` を呼び出すと、レシピを登録したプラグインの `PluginMeta` を取得できます。
CustomCrafterAPI 自身が登録した場合は `null` を返します (5.2.0 以降)。
