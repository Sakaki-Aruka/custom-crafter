...
### CoordinateComponent のファクトリメソッド
...
:::note
- `predicates` が持つ `CRecipePredicate` については[CRecipePredicate のページ](/ja/recipe/predicate/)を参照してください。
- `results` が持つ `ResultSupplier` については[成果物のページ](/ja/recipe/result/)を参照してください。
:::