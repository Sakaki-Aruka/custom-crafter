...
public class SettingsLoad {
    ...
    public void set(){
        ...
        defaultConfig = CustomCrafter.getInstance().getConfig();
        ...
        if(defaultConfig.contains("download")){
            if(!defaultConfig.getStringList("download").isEmpty()){
                ...
                downloader.runTaskAsynchronously(CustomCrafter.getInstance());
                defaultConfig.set("download",failed);
                CustomCrafter.getInstance().saveConfig();
            }else{
                main.runTaskLater(CustomCrafter.getInstance(),20);
                return;
            }
        }
        ...
        }.runTaskTimer(CustomCrafter.getInstance(),20,load_interval);
    ...
    private void getEnchantedMaterials(Path path){
        FileConfiguration config = YamlConfiguration.loadConfiguration(path.toFile());
        String name = config.getString("name");
        ItemStack itemStack = new ItemStack(Material.valueOf(config.getString("material").toUpperCase()),config.getInt("amount"));
        EnchantedMaterial item = new EnchantedMaterial(itemStack);
