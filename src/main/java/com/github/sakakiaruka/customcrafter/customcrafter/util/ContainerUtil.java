...
public class ContainerUtil {
    ...
    public static final TriConsumer<Map<String, String>, ItemStack, String> SPAWN_EGG = (data, item, formula) -> {
        ...
        // actions
        // - addPassenger=(name)
        // - addPotionEffect=(potionInternal's formula)
        // - setAI (bool)
        ...
        // - setPose (Pose)
        // - serRotation (float yaw, float pitch)
        // - setSilent (bool)
        // - setSneaking (bool)
        ...
        // EntitySnapshot.createEntity(World) -> cast to Mob class -> getEquipment -> setChest, setBoots, .....
        // - setBoots
        // - setChestPlate
        // - setHelmet
        // - setLeggings
        ...
        // - setOffHandDropChance
        formula = CalcUtil.getContent(data, formula);
        final String pattern = "base: (random:\\[[a-zA-Z_0-9!,]+]|[a-zA-Z0-9_]+)(,.+)?";
        Matcher parsed = Pattern.compile(pattern).matcher(formula);
        if (!parsed.matches()) {
            sendIllegalTemplateWarn("spawn egg", formula, pattern);
            return;
        }
        SpawnEggMeta meta = (SpawnEggMeta) Objects.requireNonNull(item.getItemMeta());
        EntityType base = parsed.group(1).startsWith("random") ? getRandomEntityType(parsed.group(1)) : EntityType.valueOf(parsed.group(1).toUpperCase());
        if (base == null) {
            sendNoSuchTemplateWarn("entity type", parsed.group(1));
            return;
        }
        ...
        Entity p2 = world.spawn(player.getLocation(), Zombie.class);
        LivingEntity pseudo = world.createEntity(player.getLocation(), Zombie.class);
        pseudo.setAI(false);
        pseudo.setPose(Pose.FALL_FLYING);

        pseudo.getEquipment().setBoots(new ItemStack(Material.LEATHER_BOOTS), true);
        pseudo.getEquipment().setHelmet(new ItemStack(Material.NETHERITE_HELMET));
        //pseudo.addPassenger(p2.createSnapshot().createEntity(player.getLocation()));
        meta.setSpawnedEntity(pseudo.createSnapshot());
        item.setItemMeta(meta);
        p2.addPassenger(world.spawn(player.getLocation(), Zombie.class));
        meta.serialize().forEach((k, v) -> Bukkit.getLogger().info("k=" + k + ",v=" + v.toString()));

        //Bukkit.getLogger().info("passenger=" + meta.getSpawnedEntity().createEntity(player.getWorld()).getPassengers());
...
//            meta.setCustomSpawnedType(EntityType.valueOf(parsed.group(1).toUpperCase()));
//            item.setItemMeta(meta);
//            return;
        }

//        for (String element : parsed.group(2).substring(1).split(",")) {
//            if (element.isEmpty()) continue;
//            if (element.matches("passenger="))
//        }
    };
